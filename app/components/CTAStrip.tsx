...
/**
 * Oxblood closing band used at the foot of every sub-page, matching the
...
export default function CTAStrip({
  ...
  return (
    <section className="bg-accent px-[52px] py-20 max-md:px-6 max-md:py-14">
      <ScrollReveal>
        <p className="text-[11px] tracking-[0.2em] uppercase text-champagne text-center">
          {eyebrow}
        </p>
        <h2 className="mx-auto mt-4 max-w-3xl text-center font-serif text-[clamp(28px,4vw,52px)] leading-tight text-bg">
          {title}
        </h2>
        <p className="mx-auto mt-4 max-w-xl text-center text-[15px] leading-relaxed text-[rgba(244,242,238,0.75)]">
          {copy}
        </p>
        <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
          <a
            ...
            className="inline-flex items-center justify-center gap-3 bg-bg px-8 py-4 font-serif text-[18px] uppercase tracking-[0.08em] text-accent transition hover:brightness-95"
          >
          ...
          {secondary ? (
            <Link
              href={secondary.href}
              className="inline-flex items-center justify-center border border-[rgba(244,242,238,0.4)] px-8 py-4 text-[11px] uppercase tracking-[0.18em] text-bg transition hover:border-bg"
            >
              {secondary.label}
            </Link>
          ) : null}
