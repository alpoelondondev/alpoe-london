...
export default function WhatsAppButton() {
  ...
  return (
    <a
      ...
      // breakpoint — so this is bottom-8 on a desktop and sits clear of the bar
      // on a phone, home indicator included. The gap is 24px rather than the
      // bar's own 12px float: the badge grew, and at 12px the disc and the
      // pill were close enough to read as one crowded corner.
      className="group fixed bottom-[calc(var(--tab-h)+24px)] right-6 z-[100] flex shadow-lg shadow-black/50 rounded-full transition-transform duration-200 hover:scale-105 md:bottom-8"
    >
      {/* The halo, on the same centre and behind the disc. It is the badge's
          own green at a low alpha, so it reads as the control breathing rather
          than as a second element sitting under it, and it is
          `pointer-events-none` so the thing under the thumb is still the disc.
          Reduced motion drops it entirely — see .wa-halo in globals.css. */}
      <span
        aria-hidden="true"
      ...
      {/* Turns a half-circle clockwise. `group-active` covers touch, where
          ...
          stuck :hover would otherwise leave the badge upside down after the
          tap. */}
      <span className="relative flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-[#25D366] transition-transform duration-500 ease-out group-hover:rotate-180 group-active:rotate-180 max-md:h-14 max-md:w-14">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 32 32"
          className="h-9 w-9 fill-white max-md:h-8 max-md:w-8"
          aria-hidden="true"
        >
