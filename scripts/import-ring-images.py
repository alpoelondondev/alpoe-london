#!/usr/bin/env python3
"""
Imports the engraved ring artwork into public/rings/, for the Ring Collection.
...
Note that these are NOT the ring builder's pictures any more. See the comment
on COLLECTION_MANIFEST below.
...
# builder does not have yet — which meant every configuration resolved to the
# same round platinum solitaire regardless of what the customer had chosen.
# A picture that contradicts the specification beside it is worse than no
...
def write_manifest():
    ...
    for folder, kind, target in (
        ...
        for name in sorted(os.listdir(folder)):
            ...
            manifests[target].append((f"{kind}/{stem}", hashed))
